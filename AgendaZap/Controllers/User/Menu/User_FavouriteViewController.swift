//
//  User_FavouriteViewController.swift
//  AgendaZap
//
...
class User_FavouriteViewController: UIViewController, UIScrollViewDelegate, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate
{
    // MARK: - IBOutlet
    ...
    let strScreenID: String = "4"
    
    var dataRows = [FavouriteStore]()
    ...
    override func viewDidLoad() {
        ...
        self.dataRows = [FavouriteStore]()
        
        for obj in realm.objects(FavouriteStore.self)
        {
...
//    }
    ...
    @objc func user_GetAllFavouriteStoresEvent()
    {
        DispatchQueue.main.async(execute: {
            ...
            let realm = try! Realm()
            try! realm.write({
                realm.delete(realm.objects(FavouriteStore.self))
    ...
    func setUpNavigationBar()
    {
        ...
        navigationTitle?.text = NSLocalizedString("menu_favourites", value:"Favoritos", comment: "")
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat
    {
        if (dataRows.count > 0)
        {
            ...
            cell.lblName.text = dataRows[indexPath.row].strName
            cell.lblTag.text = dataRows[indexPath.row].strTags
            ...
            if (dataRows[indexPath.row].strPromo == "")
            {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        viewController.strSelectedStoreID = dataRows[indexPath.row].strID
    ...
    @IBAction func btnSendClicked(_ sender: UIButton)
    {
        ...
        dataManager.user_UpdateStoreClicks(strStoreID: dataRows[sender.tag].strID, strScreenID: self.strScreenID)
        
        if (dataRows[sender.tag].strPhoneNumber != "")
        {
            let originalString = "https://wa.me/55\(dataRows[sender.tag].strPhoneNumber)?text=Olá. Achei seu contato no AZpop (Cliente n. \(UserDefaults.standard.value(forKey: "hashed_user_id") ?? "-"))&source=&data="
            let escapedString = originalString.addingPercentEncoding(withAllowedCharacters:NSCharacterSet.urlQueryAllowed)
           // print(escapedString as Any)
