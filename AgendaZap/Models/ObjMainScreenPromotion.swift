...
class ObjMainScreenPromotion: NSObject
{
    var strID: String = ""
    ...
    func separateParametersForMainScreenPromotion(dictionary :Dictionary<String, Any>)
    {
        strID = "\(dictionary["id"] ?? "")"
        strCityID = "\(dictionary["city_id"] ?? "")"
        strType = "\(dictionary["type"] ?? "")"
        StrSortOrder = "\(dictionary["sort_order"] ?? "")"
        strIconID = "\(dictionary["icon_id"] ?? "")"
